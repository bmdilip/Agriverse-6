...
import { Wallet, TrendingUp, Leaf, Award, Eye, Download, Camera, BarChart3, DollarSign, Calendar, TreePine, Users, Shield, Edit3, ExternalLink, PlayCircle, Coins, Target, Zap, Bell, Filter, Cog as Cow, Bird, Bone } from 'lucide-react';
        ...
        >
          ...
          {activeTab === 'yields' && (
            <div className="bg-agri-card border border-agri-border rounded-2xl p-6">
              ...
              <div className="h-96 bg-agri-secondary/20 rounded-xl flex items-center justify-center mb-6">
                <div className="text-center">
                  <TrendingUp className="w-16 h-16 text-agri-primary mx-auto mb-4" />
                  <p className="text-agri-text/70 text-lg">Interactive ROI chart visualization</p>
          ...
          {activeTab === 'live-feeds' && (
            <div className="space-y-6">
              <h3 className="text-2xl font-light text-agri-text">Your Farm Live Feeds</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {userNFTs.filter(nft => nft.liveFeed && nft.liveStreamUrl).map((nft) => (
                  <div key={nft.id} className="bg-agri-card border border-agri-border rounded-2xl overflow-hidden">
                    <div className="p-4 border-b border-agri-border">
                      <div className="flex items-center justify-between">
                        <h4 className="text-lg font-medium text-agri-text">{nft.title}</h4>
                        <div className="flex items-center space-x-2 bg-red-500/20 rounded-full px-3 py-1">
                    ...
                    <div className="relative">
                      <iframe
                        src={nft.liveStreamUrl}
                        className="w-full h-64"
                        frameBorder="0"
                        allowFullScreen
                        title={`Live feed for ${nft.title}`}
                      />
                      <div className="absolute bottom-4 left-4 right-4">
                        <div className="bg-agri-dark/80 backdrop-blur-sm rounded-lg p-3">
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="text-agri-text font-medium">{nft.type}</div>
                              <div className="text-agri-text/70 text-sm">Maturity: {nft.maturity}%</div>
                            </div>
                            <button className="px-3 py-1 bg-agri-primary text-agri-dark rounded-lg text-sm hover:bg-agri-primary/90 transition-colors">
                              Fullscreen
                            </button>
                          </div>
                        </div>
              ...
              {userNFTs.filter(nft => nft.liveFeed && nft.liveStreamUrl).length === 0 && (
                <div className="text-center py-12">
                  <Camera className="w-16 h-16 text-agri-text/50 mx-auto mb-4" />
                  <p className="text-agri-text/70">No live feeds available for your NFTs</p>
                </div>
              )}
            </div>
          )}
